...
if ($GLOBALS["MINIME_T5"]) {
    if (isset($FEATURES["MISC"]["OGHMA_INFINIUM"])&&($FEATURES["MISC"]["OGHMA_INFINIUM"])) {
        ...
        //    $topic=json_decode(file_get_contents("http://127.0.0.1:8082/posttopic?text=".urlencode($TEST_TEXT)),true);
            $topic=json_decode(file_get_contents($GLOBALS["MINIME_T5_URL"] . "/posttopic?text=" . urlencode($TEST_TEXT)),true);
...
if ($GLOBALS["MINIME_T5"]) {
    ...
    //    $command=json_decode(file_get_contents("http://127.0.0.1:8082/task?text=".urlencode($TEST_TEXT)),true);
        $command=json_decode(file_get_contents($GLOBALS["MINIME_T5_URL"] . "/task?text=" . urlencode($TEST_TEXT)),true);
